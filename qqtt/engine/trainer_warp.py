...
class InvPhyTrainerWarp:
    def __init__(
        ...
        self.simulator = SpringMassSystemWarp(
            ...
            k_base_from_rest_length=getattr(cfg, "k_base_from_rest_length", True),
            k_base_scale=getattr(cfg, "k_base_scale", 1.0),
            rest_length_clamp_min=getattr(cfg, "rest_length_clamp_min", 1e-4),
            edge_gate_init=getattr(cfg, "edge_gate_init", 3.0),
        )
        ...
        if not pure_inference_mode:
            ...
            if use_edge_gating:
                optim_params = [
                    wp.to_torch(self.simulator.wp_edge_gate),
                    wp.to_torch(self.simulator.wp_log_a),
                ] + optim_params
            else:
                optim_params = [wp.to_torch(self.simulator.wp_spring_Y)] + optim_params
    ...
    def train(self, start_epoch=-1):
        ...
        for i in range(start_epoch + 1, cfg.iterations):
            ...
            if i % cfg.vis_interval == 0 or i == cfg.iterations - 1:
                ...
                if getattr(self.simulator, "use_edge_gating", False) and self.simulator.wp_edge_gate is not None:
                    cur_model["edge_gate"] = wp.to_torch(
                        self.simulator.wp_edge_gate, requires_grad=False
                    )
                    cur_model["log_a"] = wp.to_torch(
                        self.simulator.wp_log_a, requires_grad=False
                    )
    ...
    def test(self, model_path=None):
        if model_path is not None:
            ...
            if "edge_gate" in checkpoint and "log_a" in checkpoint and self.simulator.wp_edge_gate is not None:
                self.simulator.set_edge_gate(checkpoint["edge_gate"].detach().clone().to(cfg.device))
