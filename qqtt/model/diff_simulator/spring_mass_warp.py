...
def eval_springs_gated(
    ...
    tid = wp.tid()
    # sigmoid(gate): w_ij in [0, 1]
    w_ij = 1.0 / (1.0 + wp.exp(-edge_gate[tid]))
    a_ij = wp.exp(log_a[tid])
    k_eff = w_ij * a_ij * k_base[tid]
    if k_eff <= spring_Y_min:
        return
    k_eff = wp.min(k_eff, spring_Y_max)
...
class SpringMassSystemWarp:
    def __init__(
        ...
        k_base_from_rest_length=True,
        k_base_scale=1.0,
        rest_length_clamp_min=1e-4,
        edge_gate_init=3.0,
    ):
        ...
        if use_edge_gating:
            self.wp_edge_gate = wp.from_torch(
                torch.full(
                    (self.n_springs,),
                    edge_gate_init,
                    dtype=torch.float32,
            ...
            self.wp_log_a = wp.from_torch(
                torch.zeros(self.n_springs, dtype=torch.float32, device=self.device),
                requires_grad=True,
            )
            rest_lens = init_rest_lengths.detach().cpu().numpy()
            rest_clamped = np.clip(rest_lens, rest_length_clamp_min, None)
            k_base_vals = (k_base_scale / rest_clamped).astype(np.float32)
            self.wp_k_base = wp.from_torch(
                torch.tensor(k_base_vals, dtype=torch.float32, device=self.device),
                requires_grad=False,
            )
        else:
            self.wp_edge_gate = None
            self.wp_log_a = None
            self.wp_k_base = None
    ...
    def set_edge_gate(self, gate_tensor):
        if self.wp_edge_gate is None:
            return
        src = wp.from_torch(gate_tensor.contiguous().float(), requires_grad=False)
        wp.launch(copy_float, dim=self.n_springs, inputs=[src], outputs=[self.wp_edge_gate])

    def set_log_a(self, log_a_tensor):
        if self.wp_log_a is None:
            return
        src = wp.from_torch(log_a_tensor.contiguous().float(), requires_grad=False)
        wp.launch(copy_float, dim=self.n_springs, inputs=[src], outputs=[self.wp_log_a])
