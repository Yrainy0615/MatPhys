...
def _load_original_playground_cfg(
    ...
    # Keep legacy topology so existing physics checkpoints match spring count.
...
def main():
    parser = argparse.ArgumentParser(
        description="Launch object-point drag playground with explicit physics and Gaussian checkpoints"
    )
    ...
    parser.add_argument("--bg_img_path", default=None)
    parser.add_argument(
        "--method",
        choices=["controller", "point"],
        default="controller",
    )
    ...
    if args.bg_img_path is not None:
        ...
        cfg.bg_img_path = os.path.join(case_dir, "color", "0", "0.png")
    ...
    print("[info] Launching object drag playground")
    print("[info] Double click: select nearest object point")
    print("[info] Hold left mouse button and drag: interact")
    print("[info] Right click: clear selection")
    print("[info] ESC: quit")
